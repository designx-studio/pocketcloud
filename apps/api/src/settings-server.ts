...
app.get('/api/v1/settings', async (req) => { await seedDefaultSettings(prisma, config.POCKETCLOUD_DOMAIN); return listSettings(prisma); });
app.put('/api/v1/settings/:key', async (req, reply) => {
  ...
  try { return await updateSetting(prisma, (req as any).auth.userId, req.ip, input); }
  catch (error) { const message = error instanceof Error ? error.message : 'invalid_setting'; return reply.code(message === 'unsupported_setting' ? 404 : 400).send({ error: message }); }
});
app.get('/health', async () => ({ status: 'ok', service: 'pocketcloud-settings' }));
app.listen({ port: 8082, host: '0.0.0.0' }).catch((error) => { app.log.error(error); process.exit(1); });
