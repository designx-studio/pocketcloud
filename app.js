...
function switchView(v){const l=$('viewLanding'),d=$('viewDashboard');if(v==='dashboard'&&!auth.accessToken)return $('modalAuth')?.classList.remove('hidden');l?.classList.toggle('hidden',v==='dashboard');d?.classList.toggle('hidden',v!=='dashboard');state.activeView=v;if(v==='dashboard'){startPolling();loadDashboard()}iconRefresh()}function startPolling(){if(!state.polling)state.polling=setInterval(()=>loadServers().catch(()=>{}),5000)}function stopPolling(){if(state.polling)clearInterval(state.polling);state.polling=null}
...
async function selectServer(id, refreshList = true){
  const s=state.servers.find(x=>x.id===id);
  if(!s)return;
  state.selectedServer=s;
  $('boxReinstallAgentCmd')?.classList.add('hidden');
  if($('drawerNodeName'))$('drawerNodeName').textContent=s.name;
  if($('drawerNodeMeta'))$('drawerNodeMeta').textContent=`${s.provider} • ${s.ipAddress} • ${s.os}`;
  if($('terminalNodeLabel'))$('terminalNodeLabel').textContent=`root@${s.ipAddress}:~#`;
  $('detailEmptyState')?.classList.add('hidden');
  $('nodeDetailDrawer')?.classList.remove('hidden');
  if(refreshList) {
    renderServers(state.servers);
    resetGauges();
    if ($('txtTerminalConsole')) $('txtTerminalConsole').textContent = '> Ready. Select a node command above or type a custom command to execute remotely via outbound agent.';
    if ($('taskConsoleOutput')) $('taskConsoleOutput').textContent = '> No task dispatched yet.';
    if ($('taskPipelineStatus')) $('taskPipelineStatus').textContent = 'IDLE';
    if ($('taskProgressBarFill')) $('taskProgressBarFill').style.width = '0%';
    if (taskTrackerTimer) {
  ...
  if ($('detailNodeStatus')) {
    ...
    $('detailNodeStatus').className = `status-chip ${isOnline ? 'online' : 'offline'}`;
    $('detailNodeStatus').textContent = s.status;
  }
  
  try{
    const metrics = await get(`/api/v1/servers/${id}/metrics?limit=1`);
    ...
    if($('gaugeCpuVal')) $('gaugeCpuVal').textContent = `${Number(cpuVal).toFixed(1)}%`;
    if($('gaugeCpuFill')) $('gaugeCpuFill').style.width = `${Math.max(0, Math.min(100, Number(cpuVal)))}%`;
    if($('gaugeCpuSub')) $('gaugeCpuSub').textContent = `Load: ${m.load !== undefined ? Number(m.load).toFixed(2) : '—'}`;
    
    // RAM
    const ramVal = m.memory ?? 0;
    if($('gaugeRamVal')) $('gaugeRamVal').textContent = `${Number(ramVal).toFixed(1)}%`;
    if($('gaugeRamFill')) $('gaugeRamFill').style.width = `${Math.max(0, Math.min(100, Number(ramVal)))}%`;
    if($('gaugeRamSub')) {
      if (m.memUsedMb !== undefined && m.memTotalMb !== undefined && m.memTotalMb > 0) {
        const usedStr = m.memUsedMb >= 1024 ? `${(m.memUsedMb/1024).toFixed(1)} GB` : `${Math.round(m.memUsedMb)} MB`;
        const totStr = m.memTotalMb >= 1024 ? `${(m.memTotalMb/1024).toFixed(1)} GB` : `${Math.round(m.memTotalMb)} MB`;
        $('gaugeRamSub').textContent = `${usedStr} / ${totStr}`;
      } else {
        $('gaugeRamSub').textContent = 'Live telemetry';
      }
    }
    
    // Disk
    const diskVal = m.disk ?? 0;
    if($('gaugeDiskVal')) $('gaugeDiskVal').textContent = `${Number(diskVal).toFixed(1)}%`;
    if($('gaugeDiskFill')) $('gaugeDiskFill').style.width = `${Math.max(0, Math.min(100, Number(diskVal)))}%`;
    if($('gaugeDiskSub')) {
      if (m.diskUsedGb !== undefined && m.diskTotalGb !== undefined && m.diskTotalGb > 0) {
        $('gaugeDiskSub').textContent = `${Number(m.diskUsedGb).toFixed(1)} GB / ${Number(m.diskTotalGb).toFixed(1)} GB`;
      } else {
        $('gaugeDiskSub').textContent = 'Live telemetry';
      }
    }
  }catch(e){
    resetGauges();
...
function resetGauges(){
  ['Cpu','Ram','Disk'].forEach(k=>{
    if($(`gauge${k}Val`))$(`gauge${k}Val`).textContent='Awaiting data';
    if($(`gauge${k}Fill`))$(`gauge${k}Fill`).style.width='0%';
  });
  if($('gaugeCpuSub')) $('gaugeCpuSub').textContent = 'Load: —';
  if($('gaugeRamSub')) $('gaugeRamSub').textContent = '— / —';
  if($('gaugeDiskSub')) $('gaugeDiskSub').textContent = '— / —';
}
...
async function trackTerminalTask(taskId, cmd) {
  const consoleBox = $('txtTerminalConsole');
  let pollCount = 0;
  const interval = setInterval(async () => {
    ...
    try {
      ...
      const logs = await get(`/api/v1/tasks/${taskId}/logs`).catch(() => []);
      if (consoleBox) {
        const logLines = logs.map(l => l.message).join('\n');
        consoleBox.textContent = `root@${state.selectedServer?.ipAddress || 'node'}:~# ${cmd}\n\n${logLines || `[Status: ${task.status}] Executing payload...`}`;
      }
      if (task.status === 'SUCCESS' || task.status === 'FAILED' || pollCount > 30) {
...
function bindEvents(){
  ...
  $('formWebTerminal')?.addEventListener('submit', async e => {
    e.preventDefault();
    const input = $('inputTerminalCmd');
    const cmd = input?.value.trim();
    if (!cmd) return;
    if (!state.selectedServer) return toast('Select a server node first', 'error');

    const consoleBox = $('txtTerminalConsole');
    if (consoleBox) consoleBox.textContent = `root@${state.selectedServer.ipAddress}:~# ${cmd}\n> Executing command via outbound agent...`;
    
    try {
      const task = await post('/api/v1/tasks', {
        serverId: state.selectedServer.id,
        type: 'exec',
        payload: { command: cmd }
      });
      toast('Command dispatched to agent', 'success');
      trackTerminalTask(task.id, cmd);
    } catch (err) {
  ...
  document.querySelectorAll('.btn-term-quick').forEach(b => {
    b.addEventListener('click', () => {
      const cmd = b.dataset.cmd;
      if ($('inputTerminalCmd')) $('inputTerminalCmd').value = cmd;
      $('formWebTerminal')?.dispatchEvent(new Event('submit'));
    });
  });

  $('btnClearTermLogs')?.addEventListener('click', () => {
    if ($('txtTerminalConsole')) $('txtTerminalConsole').textContent = '> Terminal console logs cleared.';
  });
