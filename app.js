...
async function checkSession() {
  try {
    ...
    // No usable refresh cookie: stay on the landing view.
    console.info('No active session restored:', e.message);
  }
}
function copyToClipboard(text,successMessage){
  ...
  navigator.clipboard.writeText(text)
    ...
    .catch(err=>{console.error('Clipboard write failed:',err);toast('Copy failed — select the command and copy it manually.','error')});
}
function bindEvents(){
  ...
  $('btnCopyInstallSection')?.addEventListener('click', () => {
    copyToClipboard($('txtAgentInstallSectionCmd')?.textContent, 'Copied agent installation command!');
  });

  $('btnCopyControlPlaneInstall')?.addEventListener('click', () => {
    copyToClipboard($('txtControlPlaneInstallCmd')?.textContent, 'Copied deploy command!');
  });
  ...
  $('btnCopyReinstallAgentCmd')?.addEventListener('click', () => {
    copyToClipboard($('txtReinstallAgentCmd')?.textContent, 'Copied reinstallation command!');
  });
}
